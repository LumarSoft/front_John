...
import { slideLeft, slideRight, stagger, fadeUp, viewport } from '@/src/lib/motion'
      ...
      <div className="container py-[120px]">
        <div className="grid grid-cols-2 gap-16 items-start max-[880px]:grid-cols-1 max-[880px]:gap-10">
          <motion.div initial="hidden" whileInView="visible" viewport={viewport} variants={stagger(0.1)}>
            <motion.div
              variants={fadeUp}
              className="text-[11px] tracking-[0.32em] uppercase text-amber font-medium mb-7"
            >
              Dónde estamos
            </motion.div>
            <motion.h2
              variants={slideLeft}
              className="font-bold text-[clamp(36px,4.8vw,60px)] leading-[1.02] tracking-[-0.045em] text-cream m-0 mb-10"
            >
              Visitanos en <em className="not-italic text-amber">Rosario.</em>
            </motion.h2>

            <motion.div variants={stagger(0.1)} className="flex flex-col gap-8">
          ...
          <motion.div
            className="rounded-2xl overflow-hidden border border-line-2 h-[440px] max-[880px]:h-[300px]"
            initial="hidden"
            whileInView="visible"
            viewport={viewport}
            variants={slideRight}
